...
#         x = self.norm(x + residual)  # 残差连接后再归一化
#         out = self.output_layer(x)
#         return out

class ProjectorTwoLayer(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(ProjectorTwoLayer, self).__init__()
        self.linear1 = nn.Linear(input_dim, output_dim)
        self.linear2 = nn.Linear(output_dim, output_dim)

    def forward(self, x):
        x = F.relu(self.linear1(x))
        out = self.linear2(x) + x  # 残差连接
        return out
...
class PLModel(pl.LightningModule):
    def __init__(self, model,config,train_loader,test_loader):
        ...
        self.criterion = ClipLoss(temperature=config['temperature'])
        ...
        # 定义投影层
        # self.eeg_projector = Projector(self.z_dim, 512)
        # self.image_projector = Projector(self.z_dim, 512)
    ...
    def forward(self, batch,sample_posterior=False):
        ...
        # 投影到joint feature space
        img_z = self.eeg_projector(img_z)
        eeg_z = self.image_projector(eeg_z)
        
        # img_z = img_z/img_z.norm(dim=-1, keepdim=True)

        logit_scale = self.criterion.logit_scale
        logit_scale = self.criterion.softplus(logit_scale)
